    ...
    fn parse() -> Self {
        ...
        while i < argv.len() {
            match argv[i].as_str() {
                ...
                "--rng" => {
                    i += 1;
                    out.rng_filters.push(argv.get(i).unwrap_or_else(|| die("--rng requires an argument")).clone());
                }
                "--hc-n" => {
                    i += 1;
...
fn run_case(label: &str, mut rng: impl Rng, args: &Args) {
    ...
    let hi = hamming_indep(&mut rng, args.hi_n, args.hi_r, args.hi_s, args.hi_l, args.hi_d);
